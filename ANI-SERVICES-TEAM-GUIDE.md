...
| **Arco Design React**（`@arco-design/web-react`） | UI 组件库（**唯一指定**，禁止混用 TDesign / Ant Design / MUI 等） |
| `@arco-design/web-react/icon` | Arco 图标库 |
...
- UI 组件优先使用 Arco Design React 提供的组件，不引入其他 UI 框架
- 颜色、间距、字体使用 **Arco Design Token**；壳层布局可使用 **Tailwind utilities**（见 `产品设计规范-样式与Tailwind边界-2.0.md`）
...
**当前状态：**  
目录存在但为空，需要 Services 团队从零搭建。技术栈和目录结构建议与 `console/` 保持一致（相同的 React + **Arco Design React** 技术栈；设计规范见 `UI规范-2.0.md`）。
