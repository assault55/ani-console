...
进度状态冲突时，以 `ANI-06-开发计划.md` Section 零和 `repo/CURRENT-SPRINT.md` 为准。工程约定冲突时，以本文的强制规则为准。h 

---

## 3. 强制架构边界

1. ANI 分为 ANI Core 和 ANI Services 两层。ANI Core 只负责基础设施平台能力，跨层控制面契约只输出 Core OpenAPI REST API、Core SDK、CLI；不得包含模型推理、RAG、PaaS 业务逻辑,这里只有前端项目 后端项目路径：`/root/kubercon/ANI`。
...
## 9. Console 前端与设计规范冻结（design 仓库）

`frontends/console/` 为**独立前端工程**，产品视觉以冻结的设计规范 2.0 为只读真来源。

1. **设计规范 2.0 已于 2026-06-25 冻结**：禁止修改 `UI规范-2.0.md`、`产品设计规范-*-2.0.md` 正文。见 [DESIGN-SPEC-FREEZE.md](./DESIGN-SPEC-FREEZE.md)。
2. **Console 开发入口**：[frontends/console/CLAUDE.md](./frontends/console/CLAUDE.md) + [CONVENTIONS.md](./frontends/console/CONVENTIONS.md)。
3. **存量页面改造**：按 [CONSOLE-SPEC-COMPLIANCE-BATCHES.md](./frontends/console/docs/CONSOLE-SPEC-COMPLIANCE-BATCHES.md) **SCB-01 → SCB-12** 顺序执行，不得跳批。当前：**SCB-04**。
4. **新页面**：先只读规范选定页面模板，再写代码；每批 `npm run verify`。
5. **进度**：只写入 `CONSOLE-TASK-PLAN.md`、`SPRINT-SCB-*.md`，不回写冻结规范。当前批次：**SCB-04**。
