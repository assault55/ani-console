...
`frontends/console/` 内页面组件 **必须与 `createFileRoute` 写在同一路由文件**（`src/routes/**/*.tsx`）。

| 目录 | 用途 |
|------|------|
| `src/routes/` | 路由定义 + 页面组件（同文件） |
| `src/components/` | 跨页面复用的 Shell、表格、CRUD 封装 |
| `src/api/` | OpenAPI 生成类型与 `coreApi` |

**禁止** 使用 `src/pages/` 单独存放页面；**禁止** 路由文件仅 `import` 外部页面 re-export。  
详细约定与评审清单见 `frontends/console/CONVENTIONS.md`；分阶段交付记录见 `frontends/console/docs/CONSOLE-SPRINT-PHASES.md`。

**测试（强制）**：每个功能 Sprint 结束前必须 `cd frontends/console && npm run verify`（单元测试 + E2E + build 全通过）。不得仅构建或仅手测。见 `CONVENTIONS.md` §4.2。
