...
- **Console**（用户控制台）& **BOSS**（运营运维后台）：React 18 + TanStack Router + TanStack Query + **Arco Design React**
- 从 API 契约自动生成 TypeScript SDK，前端禁止手写 API 调用 URL
- 移动端（Phase 3）：React Native + 与 Web 对齐的设计 Token 策略，与 Web 共享业务逻辑和类型定义
...
| Arco Design React | **2.x** | UI 组件库（Console & BOSS **唯一指定**） |
| Tailwind CSS | **4.x** | **仅布局 utilities**（preflight 关闭；与 Arco 混用见样式边界文档） |
| Vite | **5+** | 前端构建工具 |
